import { 
  ...
  type EventChallenge,
  type InsertEventChallenge,
  type UserEventProgress,
  type InsertUserEventProgress,
  type UserChallengeProgress,
  type InsertUserChallengeProgress,
  type EventCard,
  type InsertEventCard,
  users,
  posts,
  postLikes,
  cards,
...
export class DbStorage implements IStorage {
  ...
  async getActiveEvents(): Promise<SeasonalEvent[]> {
    ...
    return db
      ...
      .orderBy(desc(seasonalEvents.isFeatured), asc(seasonalEvents.endDate));
  }
  ...
  async getEventBySlug(slug: string): Promise<SeasonalEvent | undefined> {
    const result = await db
      .select()
      .from(seasonalEvents)
      .where(eq(seasonalEvents.slug, slug))
      .limit(1);
    return result[0];
  }

  async getEventById(id: string): Promise<SeasonalEvent | undefined> {
    const result = await db
      .select()
      .from(seasonalEvents)
      .where(eq(seasonalEvents.id, id))
      .limit(1);
    return result[0];
  }

  async createEvent(event: InsertSeasonalEvent): Promise<SeasonalEvent> {
    const result = await db.insert(seasonalEvents).values(event).returning();
    return result[0];
  }

  async updateEvent(id: string, updates: Partial<InsertSeasonalEvent>): Promise<SeasonalEvent | undefined> {
    const result = await db
      .update(seasonalEvents)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(seasonalEvents.id, id))
      .returning();
    return result[0];
  ...
  async getEventChallenges(eventId: string): Promise<EventChallenge[]> {
    return db
      .select()
      .from(eventChallenges)
      .where(and(eq(eventChallenges.eventId, eventId), eq(eventChallenges.isActive, true)))
      .orderBy(asc(eventChallenges.sortOrder));
  }

  async createEventChallenge(challenge: InsertEventChallenge): Promise<EventChallenge> {
    const result = await db.insert(eventChallenges).values(challenge).returning();
    return result[0];
  }

  async getChallengeById(id: string): Promise<EventChallenge | undefined> {
    const result = await db
      .select()
      .from(eventChallenges)
      .where(eq(eventChallenges.id, id))
      .limit(1);
    return result[0];
  }
  ...
  async getUserEventProgress(userId: string, eventId: string): Promise<UserEventProgress | undefined> {
    const result = await db
      .select()
  ...
  async joinEvent(userId: string, eventId: string): Promise<UserEventProgress> {
    const existing = await this.getUserEventProgress(userId, eventId);
    if (existing) return existing;

    const result = await db
      .insert(userEventProgress)
  ...
  async updateUserEventProgress(
    userId: string,
    eventId: string,
    updates: { totalPoints?: number; challengesCompleted?: number }
  ): Promise<UserEventProgress | undefined> {
    const result = await db
      .update(userEventProgress)
  ...
  async getEventLeaderboard(eventId: string, limit: number = 10): Promise<(UserEventProgress & { user: User })[]> {
    const results = await db
      .select()
      .from(userEventProgress)
      .innerJoin(users, eq(userEventProgress.userId, users.id))
      .where(eq(userEventProgress.eventId, eventId))
      .orderBy(desc(userEventProgress.totalPoints))
      .limit(limit);

    return results.map((r) => ({ ...r.user_event_progress, user: r.users }));
  }
  ...
  async getUserChallengesForEvent(userId: string, eventId: string): Promise<UserChallengeProgress[]> {
    const challenges = await this.getEventChallenges(eventId);
    ...
    return db
      .select()
      .from(userChallengeProgress)
      .where(and(eq(userChallengeProgress.userId, userId), inArray(userChallengeProgress.challengeId, challengeIds)));
  }

  async startChallenge(userId: string, challengeId: string): Promise<UserChallengeProgress> {
  ...
  async updateChallengeProgress(
    ...
    const current = existing?.currentProgress || 0;
    const newProgress = current + progressIncrement;
  ...
  async claimChallengeReward(userId: string, challengeId: string): Promise<boolean> {
    ...
    if (!progress || !progress.isCompleted || progress.rewardClaimed) return false;
  ...
  async getEventCards(eventId: string): Promise<(EventCard & { card: Card })[]> {
    const results = await db
      .select()
      .from(eventCards)
      .innerJoin(cards, eq(eventCards.cardId, cards.id))
      .where(eq(eventCards.eventId, eventId));

    return results.map((r) => ({ ...r.event_cards, card: r.cards }));
  }

  async addCardToEvent(eventId: string, cardId: string, dropRateBonus: number = 0, isExclusive: boolean = true): Promise<EventCard> {
    const result = await db
      .insert(eventCards)
      .values({ eventId, cardId, dropRateBonus, isExclusive })
      .returning();
    return result[0];
  }

  async getActiveEventCards(): Promise<Card[]> {
    const now = new Date();
    ...
    const results = await db
      ...
      .from(eventCards)
      .innerJoin(cards, eq(eventCards.cardId, cards.id))
      .where(inArray(eventCards.eventId, eventIds));

    return results.map((r) => r.cards);
  }
}
